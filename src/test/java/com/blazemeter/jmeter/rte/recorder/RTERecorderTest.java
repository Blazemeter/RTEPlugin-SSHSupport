...
public class RTERecorderTest {
  ...
  private void connect() throws TimeoutException, InterruptedException, RteIOException {
  ...
  public void shouldAddRteConfigToTargetControllerNodeWhenStart() throws Exception {
    ...
    ConfigTestElement expected = buildExpectedConfig(TN5250_PROTOCOL, TN5250_TERMINAL_TYPE.getId());
    assertTestElements(expected, argument.getValue());
  }

  private void assertTestElements(TestElement expected, TestElement actual) {
    PropertyIterator expectedProperties = expected.propertyIterator();
    while (expectedProperties.hasNext()) {
      JMeterProperty expectedProperty = expectedProperties.next();
      softly.assertThat(expectedProperty.getStringValue())
          .isEqualTo(actual.getPropertyAsString(expectedProperty.getName()));
    }
  }
  ...
  private ConfigTestElement buildExpectedConfig(Protocol protocol, String terminalTypeID) {
    ...
    expectedConfig.setProperty(RTESampler.CONFIG_USE_SSH, "false");
    expectedConfig.setProperty(RTESampler.CONFIG_SSH_CREDENTIALS_TYPE, "BY_PROPERTIES");
    expectedConfig.setProperty(RTESampler.CONFIG_SSH_USERNAME, "");
    expectedConfig.setProperty(RTESampler.CONFIG_SSH_PASSWORD, "");
  ...
  public void shouldAddPendingSamplerAndDisconnectSamplerToTestPlanWhenStop() throws Exception {
    ...
    verify(treeModel, times(3))
        .addComponent(argument.capture(), eq(treeNode));
    List<TestElement> testElements = argument.getAllValues();

    assertTestElements(buildExpectedConfig(TN5250_PROTOCOL, TN5250_TERMINAL_TYPE.getId()),
        testElements.get(0));
    assertTestElements(buildExpectedConnectionSampler(), testElements.get(1));
    assertTestElements(buildExpectedDisconnectionSampler(), testElements.get(2));
  }
  ...
  public void shouldAddRteConfigToTargetControllerNodeWhenStartUsingVt420() throws Exception {
    ...
    ArgumentCaptor<TestElement> argument = ArgumentCaptor.forClass(TestElement.class);
    verify(treeModel).addComponent(argument.capture(), eq(treeNode));
    assertTestElements(buildExpectedConfig(VT420_PROTOCOL, VT420_TERMINAL_TYPE.getId()), argument.getValue());
  }

  private void connectToVT420Terminal() throws TimeoutException, InterruptedException, RteIOException {
