...
public class Tn5250ClientIT extends RteProtocolClientIT<Tn5250Client> {
  ...
  private void configureClientAndConnect(SSLType type, int port) throws RteIOException, TimeoutException, InterruptedException {
    TerminalType terminalType = getDefaultTerminalType();
    client.configureClient(terminalType, type, VIRTUAL_SERVER_HOST);
    client.connect(VIRTUAL_SERVER_HOST, port, type, terminalType,
        TIMEOUT_MILLIS);
  }
  ...
  public void shouldFailWhenMatchedScreenChangedBeforeStablePeriod() throws Exception {
    ...
    TerminalType terminalType = getDefaultTerminalType();
    configureClientAndConnect(SSLType.NONE, server.getPort());
