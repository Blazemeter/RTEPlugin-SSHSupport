...
public class Tn3270ClientIT extends RteProtocolClientIT<Tn3270Client> {
  ...
  private void configureClientAndConnect(SSLType sslType, int port)
      throws RteIOException, InterruptedException, TimeoutException {
    TerminalType terminalType = getDefaultTerminalType();
    client.configureClient(terminalType, sslType, VIRTUAL_SERVER_HOST);
    client.connect(VIRTUAL_SERVER_HOST, port, sslType, terminalType, TIMEOUT_MILLIS);
  }
  ...
  private void connectExtendedProtocolClientToVirtualService()
      throws RteIOException, InterruptedException, TimeoutException {
    TerminalType terminalType = client.getTerminalTypeById("IBM-3278-M2-E");
    client.configureClient(terminalType, SSLType.NONE, VIRTUAL_SERVER_HOST);
    client.connect(VIRTUAL_SERVER_HOST, server.getPort(), SSLType.NONE,
        terminalType, TIMEOUT_MILLIS);
    awaitSync();
  }
