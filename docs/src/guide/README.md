...
![RTE Recorder](/recorder/recorder-panel.png) 
...
To start recording, the user should specify the _Server_, _Port_, _Protocol_, _Terminal Type_, _SSL Type_, _Timeout_ and _Timeout Threshold_. These configurations are the same ones detailed in [RTE-Config](#a-config-element-rte-config). Details about Timeout Threshold field and how the wait conditions works can be found [here](../../recorder/wait-conditions-recording.md).
 ...
 > - *Timeout Threshold*: This field will set the timeout which later on the [waits conditions ](../../recorder/wait-conditions-recording.md)will use to set the proper time out for the conditions.

Once everything is configured, the user proceeds to start the recording session, pressing START button. After the connection to the mainframe application is established (supposing configurations are right), the **[Terminal Emulator](../../recorder/terminal-emulator.md)** will show up. 

![alt_text](/recorder/terminal-emulator/emulator-login.png)

>Click [here](../../recorder/terminal-emulator.md) to see all the information you must know about terminal emulator.
...
![alter_text](/recorder/view-result-tree.png)

#### A Config Element (RTE Config)

![RTE Config GUI](/config.png)
...
![alt text](/rte-sampler.png "RTE Sampler GUI")
...
- *Action*. Specifies how the sampler will operate among 3 options:
  ...
  - *Send keys*. This option allows filling and submitting fields to the server with an associated attention key, wait for specified conditions and get the resulting emulator screen. If no connection is already established, this element will automatically create one.
  - *Disconnect*. This option allows to explicitly close the connection to the terminal server. This allows to restart the emulator status by reconnecting to the server en following samplers.
...
- *RTE Message*. When "Send keys" action is selected it is possible to specify fields to send and attention key to use:
  ...
  - *Positioning cursor*
    ...
    - Example of sending an Enter Attention Key at position (1, 27):
        ![alt text](/send-attention-key-at-position.png)
...
RTE Extractor is a post-processor which its purposes are to extract either positions or colors from response headers to be later used as JMeter variables.
> Check [here](../../extractor/README.md) for more information.   

#### RTE Assertion

![RTE_assertion](/assertion/rte-assertion.png)

RTE-Assertion is used to verify attributes that came from the server response headers. By this time, color is the current available attribute supported by the assertion.

Color assertion will verify, in the position previously given, that the color entered is the same one that the server response has in that position.  

>Notice: RTE-Assertion must be embedded into a sampler.

##### Supported formats to add the color:

- *Color name:* There are eight preloaded colors that came from the three protocols that the plugin supports.
Since any color name can be used to make the assertion, typing any color different from the already loaded ones, will make an "Invalid color" text to appear to warn that the color entered is not among the mentioned before.

- *Hexadecimal and RGB values:* For example: both #FF0000 and rgb(255, 0, 0) represent red color.

> Note: JMeter variables are also supported.
...
To do this, first it's required an RTE Config element specifying the server url and the protocol (TN5250). Additionally, two RTE samplers are required: one to establish the connection and validate the Welcome screen, and the other to do the login.
...
![alt text](/config.png "RTE Config")
...
![alt text](/connect.png "RTE Sampler 1")
...
![alt text](/rte-sampler.png "RTE Sampler 2") 
...
Example of usage:
  ...
  ![alter_text](/extractor/regex-extractor-date-screen.png)
...
A proper configuration in that case would be something like this: 
  ![alter_text](/extractor/regex-extractor-label-config.png)
...
Using the emulator we have identified the initial coordinates of the date value *(1,67)*. Also, we know that the length of the text would be six characters.

With all this information we have developed a regex to match that criteria:
...
![alter_text](/extractor/regex-extractor-position-config.png)
...
Otherwise, you could [contribute](../contributing/README.md) to the project. 
